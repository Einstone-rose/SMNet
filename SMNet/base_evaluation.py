...
def test(model, test_loader):
    ...
    for batch in test_loader:
        ...
        clean = Variable(batch[1].cuda()).detach()
        smap = Variable(batch[2].cuda())
        with torch.no_grad():
            prediction = model(noisy, smap).detach()
            prediction = torch.clamp(prediction, 0., 1.)
        psnr_test = psnr_test + batch_PSNR(prediction, clean, 1.)
        ssim_test = ssim_test + batch_SSIM(prediction, clean)
